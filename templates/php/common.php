...
function private_config () {
  return find_config(array("site/private.js","private.js"),'#PrivateConfig\s+=\s+#ms');
}  
...
function make_db_handle () {
  ...
  if (!  $dbhandle )  {
    $dbhandle = mysql_connect(
           $privateconfig["db"]["host"],
           $privateconfig["db"]["username"],
           $privateconfig["db"]["password"])
           or  die('Could not connect: ' . mysql_error());
    mysql_select_db($privateconfig["db"]["db"], $dbhandle) or die("Could not select database");
  }
  return $dbhandle;
