...
RUN if [ -s cicd_beta    ]; then  rsync -a -e "ssh -o StrictHostKeyChecking=no  -i cicd_release -p 2223"  output/. fskyweb@rsync.test-ipv6.com:beta/content/. || exit 1 ; fi
RUN if [ -s cicd_i18n    ]; then  rsync -a -e "ssh -o StrictHostKeyChecking=no  -i cicd_release -p 2224"  output/. fskyweb@rsync.test-ipv6.com:i18n/content/. || exit 1 ; fi
